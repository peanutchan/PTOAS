...
void LaunchVmi_quant_bf16_to_f4e1m2x2_full_kernel(uint16_t *src, uint8_t *dst,
                                                  void *stream);

int main() {
  // 256 bf16 logical lanes -> 128 packed FP4 bytes (2 f4 values per byte).
  constexpr size_t kSrcElems = 256;
  constexpr size_t kDstBytes = 128;
  size_t srcBytes = kSrcElems * sizeof(uint16_t);
  size_t dstBytes = kDstBytes * sizeof(uint8_t);
  ...
  ACL_CHECK(aclrtMemcpy(dstDevice, dstBytes, dstHost, dstBytes, ACL_MEMCPY_DEVICE_TO_HOST));
  LaunchVmi_quant_bf16_to_f4e1m2x2_full_kernel(srcDevice, dstDevice, stream);
